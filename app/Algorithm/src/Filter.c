...
void fir_arrange_filter(float *arr,unsigned short int len,unsigned char *fil_cnt,float in,float *arr_out) //len<=255 len >= 3
{
	//float arrange[len];
	float tmp;
	unsigned char i,j;
...
// #define FIL_ITEM  10
 ...
 void Moving_Average(float moavarray[],unsigned short int len ,unsigned short int *fil_cnt,float in,float *out)
{
	unsigned short int width_num;
	float last;
...
void vec_3dh_transition(float ref[VEC_XYZ], float in[VEC_XYZ], float out[VEC_XYZ]) //该函数只有在水平面附近一个有限的范围内正确。
{
	static char pn;
